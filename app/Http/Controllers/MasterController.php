...
class MasterController extends Controller {
 ...
 public function storeSample(Request $r){$data=$r->validate(['sample_code'=>'required|string|max:100|unique:samples,sample_code','sample_name'=>'nullable|string|max:255','matrix'=>'nullable|string|max:100','received_at'=>'nullable|date','customer'=>'nullable|string|max:255','notes'=>'nullable|string']); Sample::create($data); return back()->with('success','Sample berhasil disimpan.');}
 ...
 public function storeReagent(Request $r){$data=$r->validate(['code'=>'required|string|max:100|unique:reagents,code','name'=>'required|string|max:255','lot_number'=>'nullable|string|max:100','concentration'=>'nullable|numeric','unit'=>'nullable|string|max:50','expiry_date'=>'nullable|date']); Reagent::create($data); return back()->with('success','Reagen berhasil disimpan.');}
 ...
 public function storeInstrument(Request $r){$data=$r->validate(['code'=>'required|string|max:100|unique:instruments,code','name'=>'required|string|max:255','serial_number'=>'nullable|string|max:100','calibration_due'=>'nullable|date','status'=>'required|in:active,maintenance,expired']); Instrument::create($data); return back()->with('success','Instrumen berhasil disimpan.');}
