...
class UserManagementController extends Controller {
 ...
 public function store(Request $r){
  $data=$r->validate(['name'=>'required|string|max:255','email'=>['required','email','max:255',Rule::unique('lab_users','email')->where(fn($q)=>$q->where('laboratory_id',session('lab_id')))],'role'=>'required|in:admin,supervisor,analyst,viewer','password'=>'required|string|min:8','active'=>'nullable|boolean']);
 ...
 public function update(Request $r,LabUser $user){
  $data=$r->validate(['name'=>'required|string|max:255','email'=>['required','email','max:255',Rule::unique('lab_users','email')->ignore($user->id)->where(fn($q)=>$q->where('laboratory_id',session('lab_id')))],'role'=>'required|in:admin,supervisor,analyst,viewer','password'=>'nullable|string|min:8','active'=>'required|boolean']);
